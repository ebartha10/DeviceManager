...
public class UserService {
    ...
    public void deleteUser(UUID userId) {
        ...
        if(user.isEmpty()) {
            LOGGER.warn("User with id {} not found in monitoring microservice, skipping deletion", userId);
            return;
        }

        this.userRepository.deleteById(userId);
        LOGGER.debug("User with id {} was deleted from monitoring microservice", userId);
    }
}
